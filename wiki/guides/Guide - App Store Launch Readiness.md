...
| 7 | CI/CD pipeline | Not started | None exists at all — no `.github/workflows/`, no fastlane, nothing. Ties directly into [[ADR-011 - Secrets via expect-actual and local.properties]]'s already-flagged-but-unactioned "CI secret injection not set up" open thread. | A basic GitHub Actions workflow (lint + detekt + test on every push/PR at minimum; a signed-release-build job once P0 #1's signing exists) would make repeatable store updates far less error-prone. | Partially — the workflow YAML is code, but injecting the actual keystore/`local.properties`/`Secrets.xcconfig` secrets into GitHub Actions secrets is your action. |
