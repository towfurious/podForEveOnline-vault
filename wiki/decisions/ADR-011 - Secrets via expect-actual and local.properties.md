...
> **Bug found and fixed (2026-08-09)**: this ADR's own 2026-07-16 addendum predicted exactly this failure mode ("only a signed release build job... would need local.properties... injected, and only if that job needs a working SSO login rather than just a signed artifact") — and it happened. The `android-release` job wrote `keystore.properties` from secrets but never wrote `local.properties`, so every CI-built release AAB (including the one submitted to Play Store 2026-07-24) shipped with `esiClientId` defaulted to `""`. The signed AAB built and installed fine — this only breaks at runtime, the moment a real user taps "Login with EVE Online": EVE SSO immediately rejects the empty `client_id` with `{"error":"invalid_request","error_description":"The client_id parameter is required."}`. Demo Mode (see [[ADR-022 - Demo Mode]]) is unaffected since it never touches OAuth.
>
> **Found by**: installing the actual live Play Store build on a real device and hitting the broken login screen directly — not caught by any existing check, since neither `ktlintCheck`/`detekt` nor the unit-test suite nor `androidApp:assembleRelease`'s own success touches runtime OAuth behavior. An `ESI_CLIENT_ID` repo secret already existed (added at some point, presumably in anticipation of exactly this need) but no workflow step ever consumed it — same failure shape as [[ADR-023 - CI Release Keystore Debugging]]'s empty-keystore incident: a secret that exists is not the same as a secret that's actually wired into the build.
>
> **Fix**: new `Write local.properties (ESI client ID)` step in `.github/workflows/ci.yml`'s `android-release` job, right after the keystore steps — reads `secrets.ESI_CLIENT_ID`, fails fast with `::error::` if empty (mirrors the existing keystore-size guard from ADR-023), then appends `esi.client_id=<value>` to `local.properties` before `androidApp:bundleRelease` runs. Deliberately went through a GitHub secret rather than hardcoding the client ID literal into the committed YAML, even though EVE SSO `client_id` is technically public (appears in every OAuth URL) — this ADR's own Context section already rejected hardcoding it in source for exactly this project ("prevents rotation, complicates open-sourcing, violates hygiene rules"); no reason to make an exception for CI config specifically.
