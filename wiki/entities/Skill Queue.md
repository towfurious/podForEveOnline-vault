...
- The **head** of the queue (`queue_position == 0`) is *usually* the currently training skill — except right after it finishes (see next bullet), where `queue_position == 0` can be an already-completed entry for a while.
- **Completed entries remain in the response** — ESI does not immediately remove entries whose `finish_date` is in the past. The client must filter: `finishDate != null && finishDate <= now`. Without this, the UI shows a finished skill frozen at 100% progress. See `SkillQueueEntry.hasFinished(nowEpochSeconds)`. **This applies to any code that picks "the active skill," not just rendering** — `firstOrNull { it.isTraining && !it.hasFinished(now) }` (as `DashboardViewModel` and `SkillQueueRepository`'s notification scheduling both do) is the correct selector; a bare `queuePosition == 0` check picks the stale finished entry instead of the real head whenever this quirk is in effect.
...
- On finish-date reached, notification fires (Android `ForegroundService` live countdown + `AlarmManager` backup; iOS scheduled `UNNotificationRequest`) — implemented via the shared `NotificationScheduler`, see [[ADR-015 - Unified Completion Notifications]] (finishes implementing the plan from [[ADR-006 - Android Foreground Service]] / [[ADR-007 - iOS Local Notifications]]).
- Post-notification, re-fetch queue to learn new head.

## Related entities
- [[Character]] — owner.
- [[Math-Based Progress Bar]] — rendering pattern.
- [[ADR-015 - Unified Completion Notifications]] — training-complete notification behavior.
