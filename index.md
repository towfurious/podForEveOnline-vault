...
- [[ADR-018 - ESI HTTP Essentials]] — `installEsiEssentials()` on both Ktor clients: `User-Agent` header (repo URL as contact), Ktor's own `HttpCache` plugin (ESI cache-header compliance), and a custom host-scoped `EsiRateLimitPlugin` backing off on ESI's error budget / HTTP 420.
- [[ADR-019 - Offline Detection]] — synchronous `ConnectivityChecker` expect/actual (Android `ConnectivityManager`, iOS `SCNetworkReachability`) polled every 5s by `ConnectivityObserver`; `OfflineBanner` wired at the top of `App()`. Deliberately polling, not push-based callbacks — see ADR for why.
...
- [[ADR-024 - Automated Play Console Publishing]] — `android-release` now auto-publishes to the closed-testing track via `r0adkll/upload-google-play`, using a dedicated `podforeve` GCP project + service account scoped to "Release apps to testing tracks" only. `versionCode` derived from `github.run_number` instead of a hardcoded `1`. Still `workflow_dispatch`-only — no auto-publish on push.
- [[ADR-025 - Live Countdown Notification Toggle]] — `NotificationPreferences` (mirrors `ThemeRepository`, SecureStorage-backed) gates only the skill-training live-countdown shade notification, not the completion alert. Gated at `NotificationScheduler.reconcileSkillService()`, the one chokepoint both the ESI-fetch and boot-survival paths already share; tick loop self-checks every 60s so toggle-off takes effect without waiting for the next reconcile. Closes [[ADR-015 - Unified Completion Notifications]]'s last deferred P2 item.
- [[ADR-026 - Tab Back-Press Returns to Dashboard]] — new `PlatformBackHandler` expect/actual (mirrors `RequestNotificationPermissionEffect`) makes back-press from Skills/PI/Jobs return to Dashboard instead of exiting to the launcher. Root cause confirmed by reading Voyager 1.1.0-beta03's own sources: `TabNavigator` hardcodes `onBackPressed = null`. Closes a P2 item found 2026-07-16.
- [[ADR-027 - AMOLED Default and Contrast Softening]] — AMOLED is now the default theme (was Ember), per closed-testing "too bright" feedback. New `ColorScheme.softenContrast(amount)` blends every "on-*" role toward its base by one shared value, applied to all 5 themes. `amount = 0.12`, not the requested `0.20` — measured via WCAG contrast math that 0.20 pushed `onSurfaceVariant` below the 4.5:1 AA floor on 2 of 5 themes (including the new default).
...
### Resolved (2026-07-22 to 2026-07-24) — Launch Readiness closed out (except Firebase); committed + device-verified
- ~~[[Guide - App Store Launch Readiness]] P0 #1 (Android release signing)~~ — keystore generated, `signingConfigs.release` wired, real end-to-end verification (`assembleRelease`/`bundleRelease` + `apksigner verify`). Found and fixed a first-ever-release-build R8/Tink proguard gap along the way.
- ~~P0 #4 (privacy policy)~~ — published live at https://towfurious.github.io/podforeve-privacy/ (separate public repo, keeps the app repo private).
- ~~P0 #8, #9 (store listing assets, content rating questionnaire)~~ — added to the tracker as real gaps found 2026-07-22 (see [[Guide - App Store Launch Readiness]]; still open, tracked there).
- ~~P1 #8 (notification reboot survival)~~ — `BootCompletedReceiver`, see [[ADR-020 - Notification Reboot Survival]]. PI extractor notifications explicitly not re-armed (colony data never persisted).
- ~~ESI User-Agent / privacy policy contact~~ — corrected from a private (hence unreachable) GitHub repo URL to a real dedicated email, once the repo's privacy was confirmed to be staying permanent.
- **Committed** (2026-07-24): 2+ days of accumulated uncommitted work split into 5 logically-scoped commits (`2d54c8b`, `79aaec9`, `239c844`, `255358e`, `0e21882`).
- **Device-verified** (Pixel 10 Pro XL, real hardware): app installs and launches cleanly; offline banner tested end-to-end via real airplane-mode toggle. Found and fixed one real bug this way — `OfflineBanner` drew behind the status bar instead of below it (every other screen already handles this via its own `Scaffold`; the banner lives outside any screen's `Scaffold`).
- **Follow-up (2026-07-24)**: real EVE login completed on-device — all 4 screens confirmed rendering real character data correctly, faction theme switch (Gallente) confirmed including [[ADR-017 - Neon Outline Card and Icon Treatment]]'s `gainColor` amber-not-green special case, and the skill-training notification confirmed live in the shade (plus a real `304` in Chucker confirming [[ADR-018 - ESI HTTP Essentials]]'s `HttpCache` works against the real ESI server). Logout-wipe and boot-receiver reconciliation specifically still untested (need an actual logout / actual reboot, neither performed this pass). New [[verify]] skill created to make this repeatable — see `log.md` 2026-07-24.
- ~~P0 #10 (Play Console "Sign in details")~~ — new gap found 2026-07-24, closed same day: [[ADR-022 - Demo Mode]] adds `AuthState.Demo` + a static `DemoData` object so any user (or reviewer) can explore all 4 screens without an EVE account, avoiding ever sharing real EVE credentials with Google. Device-verified end-to-end: entry from `LoginScreen`, all 4 tabs, both exit paths (banner + settings sheet), real login/logout unaffected.

### Resolved (2026-07-21) — Launch Readiness P1: ESI etiquette + offline detection + logout cache wipe
- ~~[[Guide - App Store Launch Readiness]] P1 #1 (User-Agent), #2 (rate-limit/420), #3 (cache-header compliance)~~ — all done, see [[ADR-018 - ESI HTTP Essentials]]. A host-scoping bug (rate-limit gate would have blocked unrelated OAuth token-refresh calls during an ESI-side cooldown) was found and fixed during this work's own selfcheck re-review, before it ever reached a device.
- ~~[[Guide - App Store Launch Readiness]] P1 #6 (offline/connectivity detection)~~ — done, see [[ADR-019 - Offline Detection]].
- ~~[[Guide - App Store Launch Readiness]] P1 #5 (account/data deletion)~~ — confirmed logout previously left cached character data on-device (privacy policy TODO was accurate); fixed — `AuthRepository.logout()` now wipes it. See [[OAuth2 PKCE]] "Logout" section.
- **Verified**: `ktlintCheck`/`detekt` clean (both modules), `shared:testAndroidHostTest` + `shared:iosSimulatorArm64Test` green (10/10 in `EsiErrorMapperTest`, including 2 new rate-limit cases), `shared:compileAndroidMain`/`compileKotlinIosSimulatorArm64`/`composeApp:compileAndroidMain` all compile clean. Not device/simulator-verified visually this pass — `xcode-select` was found pointing at CLI-tools-only again (same issue previously resolved 2026-07-08, apparently recurred), blocking the iOS Simulator panel; flagged to the user directly rather than worked around.
- User took [[Guide - App Store Launch Readiness]]'s P0 items (signing, store forms) themselves; this work covered the P1 items codeable without further store/account setup.
